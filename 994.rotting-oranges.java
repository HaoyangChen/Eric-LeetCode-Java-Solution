...
class Solution {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (grid[i][j] == 1) {
                    ...
                    // transform the coordinate to an Integer using Math
                    queue.offer(i * n + j);
                }
            }
        }
        ...
        while (!queue.isEmpty() && fresh != 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < xBias.length; j++) {
                    ...
                    // change the fresh orange to rotten orange
                    grid[newX][newY] = 2;
                }
            }
            // increment time by 1
    ...
    // check if exceeds the boundary or if there is a fresh orange to avoid recalculation
